...
class WAVConverterApp(tk.Tk):
    def __init__(self):
       ...
       # Load and set background image
        self.background_image = Image.open("source/mov.jpg")
    ...
    def create_widgets(self):
        ...
        self.status_bar.place(x=50, y=580, width=700)

        # Information Textbox
        self.info_text = tk.Text(self, height=4, width=40, bg='black', fg='white', font=('Courier', 12))
                ...
                for file in files:
                    if file.lower().endswith(supported_extensions):
                        ...
                        if use_output_directory:
                            output_path = os.path.join(output_directory, os.path.splitext(file)[0] + ".mov")
                        else:
                            converted_folder = os.path.join(root, "Converted")
                            if not os.path.exists(converted_folder):
                                os.makedirs(converted_folder)
                            output_path = os.path.join(converted_folder, os.path.splitext(file)[0] + ".mov")
                        ...
                        self.update_idletasks()
                        command = f'ffmpeg -i "{input_path}" -acodec pcm_s16le -vcodec copy "{output_path}"'
